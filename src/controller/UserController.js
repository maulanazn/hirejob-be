...
const getAllUserController = async (req, res) => {
  ...
  if (search != undefined) {
    ...
    const resultUserSearch = await UserModel.GetallCandidateModel();
  ...
  try {
    const resultUsers = await UserModel.GetallCandidateModel();
...
const createUserController = async (req, res) => {
  ...
  try {
    ...
    // sending email
    sendToMail(result.email, 'Verify email', `<h1><a href="https://lazy-teal-piranha-vest.cyclic.cloud/user/verify/${result.id}">VERIFY EMAIL!!</a></h1>`);
